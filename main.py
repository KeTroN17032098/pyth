...
def memberlistboxUpdate():#리스트박스1의 업데이트 함수
    ...
    for i in range(1,blacklist['len']+1):
        if blacklist['members'][i]['Name']!=[""]:
            listbox1.insert(END,str(blacklist['members'][i]['Name']))
            print(str(blacklist['members'][i]['Name']))
        else:
            listbox1.insert(END,str(blacklist['members'][i]['ID']))
...
listbox1=Listbox(frame1,yscrollcommand=scrollbar1.set,width=125,height=15,font=fontStyle)#멤버 리스트박스
...
frame3.pack()
button1=Button(frame3,width=50,text="멤버 상세정보",overrelief="solid",command=showinfo,font=fontStyle)
button1.pack()
...
listbox2=Listbox(frame2,yscrollcommand=scrollbar2.set,width=25,height=0)
